"""summary
Author: Gianni Gugolz

The script is used to generate data for training a NN predictor model.

Check config_data_gen.yml for configuration options.
"""
...
from utilities.DataGen.Utilities import get_initial_states
from utilities.Recorder import create_csv_header

from f110_gym.envs.base_classes import DynamicModelPacejka, StateIndices
...
def run_data_generator(run_for_ML_Pipeline=False, record_path=None):
    ...
    dt = config['dt']
    state_history = []  # Initialize an empty list to keep track of state history
    predictions = []
    
    vehicle_dynamics_pacejka = DynamicModelPacejka(dt=config['dt'], intermediate_steps=4)
    for i in trange(len(states), desc='Processing states'):
        state = states[i]        
        s = full_state_alphabetical_to_original(state)
        control_sequence = controls[i, :, :]
        # state_history.append(s)  # Append the initial state to the history
        ...
        for control in control_sequence:
            ...
            for i in range(r):
                ...
                s = vehicle_dynamics_pacejka.step(s, control)
            
            next_state = full_state_original_to_alphabetical(s)
            state_history.append(next_state)  # Append the updated state to the history
        predictions.append(np.array(state_history))
        state_history = []
...
if __name__ == '__main__':
    ...
    run_data_generator(run_for_ML_Pipeline=True, record_path='./SI_Toolkit_ASF/Experiments/0_pacejka/Recordings')
