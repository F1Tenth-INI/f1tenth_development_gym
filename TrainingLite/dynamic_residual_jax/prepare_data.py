...
def process_data(input_csv_path, output_dir, car_param_file='gym_car_parameters.yml', dt=0.04):
    ...
    for idx in range(1, len(df)):
        ...
        # Extract current state and control from current row
        current_state = extract_state_from_row(current_row)
        control = extract_control_from_row(current_row)
        ...
        if (np.any(np.isnan(current_state)) or np.any(np.isnan(control)) or 
            np.any(np.isnan(previous_state)) or np.any(np.isnan(previous_control))):
        ...
        expected_states.append(expected_state_np)
    
    print(f"Processed {len(current_states)} valid rows out of {len(df) - 1} total rows (excluding first row)")
    ...
    for i, col in enumerate(state_cols):
        output_data[f'state_{col}'] = [s[i] for s in current_states]
    
    # Add control columns
    output_data['control_angular'] = [c[0] for c in controls]
    output_data['control_translational'] = [c[1] for c in controls]
    
    # Add expected state columns (computed from previous row's state and control)
    for i, col in enumerate(state_cols):
        output_data[f'expected_state_{col}'] = [s[i] for s in expected_states]
    
    # Add residual columns (current_state - expected_state)
    for i, col in enumerate(state_cols):
        residuals = [current_states[j][i] - expected_states[j][i] for j in range(len(current_states))]
        output_data[f'residual_state_{col}'] = residuals
...
if __name__ == "__main__":
    # Input CSV path
    input_csv = "/Users/Florian/Documents/INI/F1TENTH/f1tenth_development_gym/AnalyseData/PhysicalRecordings/2025_10_10/IPZ8_rpgd/2025-10-10_02-30-21_Recording1_0_IPZ8_rpgd-lite-jax_25Hz_vel_1.0_noise_c[0.0, 0.0]_mu_None_mu_c_None_.csv"
