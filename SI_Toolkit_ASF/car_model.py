...
from SI_Toolkit_ASF.car_equations import CarEquations
...
class car_model:
    ...
    def __init__(
        ...
        self.car_equations = CarEquations(self.lib)
    ...
    def _step_dynamics_ks(self, s, Q):
        ...
        lw_b = self.car_parameters.l_wb
        # Euler stepping
        for _ in range(self.intermediate_steps):

            s_x_dot, s_y_dot, psi_dot, v_x_dot, psi_dot_dot, delta_dot \
                = self.car_equations.ks_model(psi, v_x, delta, v_x_dot_cmd, delta_dot_cmd, lw_b)
    ...
    def _step_dynamics_pacejka(self, s, Q):
        ...
        for _ in range(self.intermediate_steps):
            d_pos_x, d_pos_y, d_psi, d_v_x, d_v_y, d_psi_dot, d_delta \
                = self.car_equations.pacejka_model(
