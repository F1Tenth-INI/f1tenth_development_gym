...
@jax.jit
def car_dynamics_pacejka_jax(state, control, car_params, dt):
    ...
    return jnp.array([psi_dot, v_x, v_y, psi, psi_cos, psi_sin, s_x, s_y, slip_angle, delta], dtype=jnp.float32)


@jax.jit
def car_dynamics_ks_pacejka_jax(state, control, car_params, dt):
    """JAX implementation of 'ODE:ks_pacejka' model with kinematic blending"""
    # First compute pure Pacejka result
    pacejka_result = car_dynamics_pacejka_jax(state, control, car_params, dt)
...
@partial(jax.jit, static_argnames=["dt", "horizon", "model_type"])
def car_steps_sequential_jax(s0, Q_sequence, car_params, dt, horizon, model_type='pacejka'):
    """
    ...
    def rollout_fn(state, control):
        next_state = dynamics_fn(state, control, car_params, dt)
...
@partial(jax.jit, static_argnames=["horizon", "model_type"])
def car_steps_sequential_with_dt_array_jax(s0, Q_sequence, car_params, dt_array, horizon, model_type='pacejka'):
    """
...
def car_batch_sequence_jax(s_batch, Q_batch_sequence, car_params, dt_array, model_type='pacejka'):
    """
    ...
    rollout_fn = lambda s, Q: car_steps_sequential_with_dt_array_jax(
        s, Q, car_params, dt_array, horizon=horizon, model_type=model_type)
    return jax.vmap(rollout_fn)(s_batch, Q_batch_sequence)


@partial(jax.jit, static_argnames=["model_type"])
def car_step_parallel_jax(states, controls, car_params, dt, model_type='pacejka'):
    """
    ...
    return jax.vmap(lambda s, c: dynamics_fn(s, c, car_params, dt))(states, controls)


# Legacy function names for backward compatibility
def car_step_jax(state, control, car_params, dt):
    """Legacy function name - use car_dynamics_pacejka_jax instead"""
    return car_dynamics_pacejka_jax(state, control, car_params, dt)


def car_step_ks_pacejka_jax(state, control, car_params, dt):
    """Legacy function name - use car_dynamics_ks_pacejka_jax instead"""
    return car_dynamics_ks_pacejka_jax(state, control, car_params, dt)
