...
class WaypointUtils:
    ...
    def __init__(self):
        ...
        self.pub_local_waypoints = rospy.Publisher('/local_waypoints/markers', MarkerArray, queue_size=10)
        ...
        time.sleep(0.5)
        self.render_global_waypoints(self.waypoints)
    ...
    def update_next_waypoints(self, car_position):
        ...
        self.render_local_waypoints(self.next_waypoints)
        
    @staticmethod
    def load_waypoints():
                
        map_name = rospy.get_param('/local_waypoints_node/map_name')
        print('loading wp for map_name', map_name)

        rospack = rospkg.RosPack()
        path = os.path.join(rospack.get_path('racecar'),'maps',map_name,map_name+'_wp.csv')


        try: # Check if waypoint file exists
            assert os.path.isfile(path), "Waypoint file (" + path+  ") does not exist"
        except AssertionError:
            print("There are no waypoints at "+path)
            print("Continuting without waypoinnts")
            return None
        
        waypoints = pd.read_csv(path, header=1).to_numpy()
    ...
    def render_global_waypoints(self, waypoints):

        p = 0
        global_waypoint_markers = MarkerArray()
        for point in waypoints:
                ...
                global_waypoint_markers.markers.append(marker)
                p += 1
        self.pub_global_waypoints.publish(global_waypoint_markers)

    def render_local_waypoints(self, waypoints):
        p = 0
        local_waypoint_markers = MarkerArray()
        for point in waypoints:
                ...
                marker.pose.position.y = point[2]
                marker.pose.orientation.w = 1
                local_waypoint_markers.markers.append(marker)
                p += 1
        self.pub_local_waypoints.publish(local_waypoint_markers)
