...
class VehicleParameters:
    ...
    motor_command_gain: float  # Motor current from commanded speed [A/(m/s)]
    motor_current_offset: float  # Motor current offset at zero command [A]
    velocity_to_current_gain: float  # Speed-dependent motor load [A/(m/s)]
    acceleration_to_current_gain: float  # Accel demand to motor current [A/(m/s^2)]
    deceleration_to_current_gain: float  # Decel/regen demand to motor current [A/(m/s^2)]
    motor_current_max_a: float  # Max drive motor current [A]
    max_brake_current: float  # Max regen/brake motor current magnitude [A]
