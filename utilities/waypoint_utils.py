...
class WaypointUtils:
    ...
    def update_next_waypoints(self, car_state):
        ...
        if(self.sectors is not None):
            for i in range(self.sectors.shape[0] - 1):
                if self.sectors[i, 1] <= nearest_waypoint_index * self.decrease_resolution_factor < self.sectors[i + 1, 1]:
                    self.sector_index = i  
                    break 
            
        print("Sector: ", self.sector_index)
    ...
    def correct_velocity(self, waypoints):
        ...
        if Settings.APPLY_SPEED_SCALING_FROM_YAML:
            ...
            self.sectors = speed_scaling_csv
                                 
            warn = False
            
            speed_scaling_array = []
            for i in range(speed_scaling_csv.shape[0]):
                start = int(speed_scaling_csv[i, 1])
                
                #Last sector goes till the end
                if(i == speed_scaling_csv.shape[0] -1 ): 
                    end = waypoints.shape[0]
                else: # normal sector
                    end = int(speed_scaling_csv[i + 1, 1])
                    
                x = speed_scaling_csv[i, 2]
                repeated_values = np.full((end - start,), x)
