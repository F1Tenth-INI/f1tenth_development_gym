...
CRITIC_PURE_TD=(True)
# CUSTOM_SAMPLING=(False True)

CUSTOM_SAMPLING=(True)

OFFSET_WEIGHTS=(2.0 3.0 4.0)
HEADINF_ERROR_WEIGHTS=(2.0 3.0 4.0)
REWARD_WEIGHTS=(2.0 3.0 4.0)
# VELOCITY_WEIGHTS=(0.0 3.0)
velocity_w=0.0

# OFFSET_WEIGHTS=(3.0 4.0)
# HEADINF_ERROR_WEIGHTS=(2.0 3.0 4.0)
# REWARD_WEIGHTS=(3.0 4.0)
          ...
          for offset_w in "${OFFSET_WEIGHTS[@]}"; do
            for heading_err_w in "${HEADINF_ERROR_WEIGHTS[@]}"; do
              for reward_w in "${REWARD_WEIGHTS[@]}"; do
                ...
                MODEL_NAME="FINAL_UTD025_250k_A${alpha}_R${ratio}_CPTD${critic_pure_td}_CU${critic_uniform}_OW${offset_w}_HW${heading_err_w}_RW${reward_w}_Cache${cache_prio}_Custom${custom_on}"
                ...
                cmd=(python -u TrainingLite/rl_racing/run_training.py
                  ...
                  --SAC_PRIORITY_CACHE_ENABLED "$cache_prio"
                )
                "${cmd[@]}"
                sleep 5
              done
