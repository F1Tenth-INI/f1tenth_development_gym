...
class mpc_planner:
    ...
    def process_observation(self, ranges=None, ego_odom=None):
        ...
        self.Render.update(self.lidar_points, self.mpc.logs['rollout_trajectories_logged'][-1],
                           self.mpc.logs['J_logged'][-1],
...
class Render:
    ...
    def render(self, e):
        ...
        if self.largest_gap_middle_point is not None:
            ...
            self.gap_vertex = shapes.Circle(scaled_points_gap_flat[0], scaled_points_gap_flat[1], 5,
                                            color=self.gap_visualization_color, batch=e.batch)
        ...
        if self.rollout_trajectory is not None:
            ...
            if self.mppi_rollouts_vertices is None:
                self.mppi_rollouts_vertices = e.batch.add(howmany_mppi, GL_POINTS, None,
                                                          ('v2f/stream', scaled_trajectory_points_flat),
        ...
        if self.optimal_trajectory is not None:
            ...
            if self.optimal_trajectory_vertices is None:
                self.optimal_trajectory_vertices = e.batch.add(howmany_mppi_optimal, GL_POINTS, None,
                                                               ('v2f/stream', scaled_optimal_trajectory_points_flat),
                                                               ('c3B',
                                                                self.optimal_trajectory_visualization_color * howmany_mppi_optimal))
            else:
        ...
        if self.target_point is not None and Settings.FOLLOW_RANDOM_TARGETS:
            ...
            self.target_vertex = shapes.Circle(scaled_target_point_flat[0], scaled_target_point_flat[1], 10,
                                               color=self.target_point_visualization_color, batch=e.batch)
...
def get_nearest_waypoints(s, all_wpts, current_wpts, nearest_waypoint_index, look_ahead=15, interpolate_waypoints=4):
    ...
    for j in range(look_ahead):
        for i in range(interpolate_waypoints):
            interpolated_waypoint = nearest_waypoints[j] + (float(i) / interpolate_waypoints) * (
                        nearest_waypoints[j + 1] - nearest_waypoints[j])
