...
class NeuralNetImitatorPlanner(template_planner):
    ...
    def __init__(self, speed_fraction=1, batch_size=1):
        ...
        self.config_training_NN = yaml.load(open(os.path.join(PATH_TO_MODELS, NET_NAME, "config_training.yml")),
    ...
    def process_observation(self, ranges=None, ego_odom=None):
        ...
        config_inputs = np.append(self.config_training_NN["training_default"]["state_inputs"],
                                  self.config_training_NN["training_default"]["control_inputs"])
        ...
        input_data = np.concatenate((self.LIDAR.processed_scans, next_waypoints_x, next_waypoints_y, next_waypoint_vx,
                                     [self.car_state[ANGULAR_VEL_Z_IDX], self.car_state[LINEAR_VEL_X_IDX],
                                      self.car_state[SLIP_ANGLE_IDX], self.car_state[STEERING_ANGLE_IDX]]), axis=0)
